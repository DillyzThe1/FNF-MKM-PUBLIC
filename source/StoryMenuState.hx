...
class StoryMenuState extends MusicBeatState
{
	...
	function updateText()
	{
		...
		var dataThing:Array<StorySongData> = [];
		for (i in 0...leWeek.songs.length)
		{
			var thisCatsName:StorySongData = {
				name: leWeek.songs[i][0],
				hidden: leWeek.songs[i].length > 3 ? leWeek.songs[i][3] : false
			};
			stringThing.push(leWeek.songs[i][0]);
			dataThing.push(thisCatsName);
		}
		...
		txtTracklist.text = '';
		for (i in 0...dataThing.length)
			if (!dataThing[i].hidden)
				txtTracklist.text += dataThing[i].name + '\n';
